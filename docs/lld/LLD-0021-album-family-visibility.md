...
- 기존 커서·날짜 필터와 같은 가족 내 작성자·보호자·시니어 해금 규칙을 유지한다. (2026-08-24 #511로 대체 — 아래 인수조건 주석을 참조하세요.)
...
- [x] 같은 가족 내부의 기존 상세 조회·해금 규칙은 유지된다.
  - 2026-08-24 #511로 내부 규칙이 변경되었습니다. 잠금 대상은 보호자가 올린 앨범뿐이며, 시니어가 올린 앨범은 해금 없이 조회합니다. 위 항목은 본 LLD 작업의 범위 한정(회귀 방지) 선언입니다.
- [x] `./gradlew :backend:widyu-api:test`가 통과한다.

## 8. 영향 범위 / 마이그레이션

기존 피드 응답의 앨범 수와 캘린더 날짜 목록이 현재 사용자의 가족 범위로 제한된다.
DB 마이그레이션과 API 계약 변경이 없다.
