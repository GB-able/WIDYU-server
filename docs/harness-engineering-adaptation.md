...
## 15. ADR/LLD 분류 및 작성 가이드

mody-server는 18개의 ADR과 20개의 LLD를 작성했다. 어떤 기준으로 나누고 어떻게 썼는지 정리한다.

---

### ADR 분류: 언제 ADR을 쓰는가

ADR은 **"이 결정을 되돌리거나 바꾸면 많은 코드가 바뀐다"** 또는 **"여러 기능에 걸쳐 반복 적용되는 규칙"** 일 때 쓴다.

판단 기준:

| 질문 | YES → | NO → |
|------|-------|-------|
| 이 결정을 나중에 바꾸면 여러 파일/모듈이 바뀌는가? | ADR | LLD 내부에 메모 |
| 여러 API/기능에 공통으로 적용되는 패턴인가? | ADR | LLD에 포함 |
| 대안이 2개 이상이고 하나를 고른 이유가 있는가? | ADR | 그냥 구현 |
| 팀 내에서 의견이 갈릴 수 있는 결정인가? | ADR | LLD에 포함 |

mody의 ADR을 유형별로 분류하면:

**① 프로젝트 초기 기반 결정 (한 번만 씀)**

| ADR | 내용 | WIDYU 상당 |
|-----|------|-----------|
| ADR-0001 | 모놀리식 단일 모듈로 시작 | 이미 widyu-api + widyu-domain으로 결정됨 |
| ADR-0002 | Spring Boot 3.4.x 안정 스택 채택 | Spring Boot 3.3.5로 결정됨 |
| ADR-0003 | DB/엔티티 설계 기준 (PK 전략, soft delete, FK 없음, enum 문자열) | 작성 필요 |
| ADR-0004 | 엔티티와 도메인 모델 통합 | widyu-domain에 이미 적용됨, 기록 필요 |

**② 여러 기능에 반복 적용되는 전략/패턴**

| ADR | 내용 | WIDYU 상당 |
|-----|------|-----------|
| ADR-0005 | OAuth 전략 패턴 (SocialLoginStrategyFactory) | 작성 필요 |
| ADR-0006 | 테스트 더블 전략 (JPA = Mock, 외부연동 = Fake) | 작성 필요 |
| ADR-0007 | 커서 기반 페이징 전략 | 작성 필요 |
| ADR-0008 | 배포 전략 | 기존 CI/CD 문서 있음 |
| ADR-0009 | 회원 인증/인가 전략 (JWT + @ValidateFamilyAccess) | 작성 필요 |

**③ 특정 기능 구현 중 발생한 어려운 결정**

| ADR | 내용 | 작성 시점 |
|-----|------|-----------|
| ADR-0010 | Presigned URL 기반 직접 업로드 | 이미지 업로드 LLD 작성 전 |
| ADR-0011 | 앱 진입 플로우 상태 플래그 분리 | 온보딩 API 구현 중 |
| ADR-0012 | 알림 발송에 트랜잭셔널 아웃박스 패턴 | 알림 발송 설계 전 |
| ADR-0013 | API 상태 소유권 분리 | 여러 API가 같은 필드 수정하는 충돌 발생 후 |
| ADR-0014 | 연속 기록 일수 계산 전략 | 계산 방식에 대안이 여러 개일 때 |
| ADR-0017 | Apple 네이티브 identity token 검증 | Apple 로그인 구현 중 방식 변경 결정 |

**결론: ADR을 쓰는 실전 판단**

```
구현하다가 "이걸 어떻게 할까?" 고민이 30분 이상 걸리면 → ADR 후보
여러 API에서 같은 결정을 반복 적용하면 → ADR
대안 비교가 필요하면 → ADR
단순히 "어떻게 만드냐"만 결정하면 → LLD
```

---

### LLD 분류: 하나의 LLD에 얼마나 담는가

LLD는 **"하나의 PR = 하나의 LLD"** 가 기본 원칙이다.  
PR 범위 기준을 그대로 LLD 범위로 삼는다.

**묶는 기준:**

```
같은 도메인의 CRUD를 한 PR에 → 하나의 LLD
  예) 그룹 생성 + 참여 + 조회 + 나가기 = LLD-0008 (그룹 API)

조회가 복잡하면 생성/수정과 분리 → 별도 LLD
  예) 기록 업로드 = LLD-0012
      기록 피드 조회 = LLD-0013 (커서 페이징, 미확인 수 포함 → 별도)

ADR에서 결정한 아키텍처를 구현할 때 → 별도 LLD
  예) ADR-0012 알림 아웃박스 결정 → LLD-0019 알림 발송 아키텍처 구현

기반 인프라는 각자 LLD
  예) JWT 로직, ID 생성기, 환경 설정 분기
```

**나누는 기준:**

```
하나의 PR에 넣기엔 diff가 너무 많다 (파일 15개 이상) → LLD 분리
의존성이 없는 두 기능이 함께 있다 → LLD 분리
한쪽만 테스트하고 싶을 때 → LLD 분리
```
...
| 유형 | 예시 | 특징 |
|------|------|------|
| 기반 인프라 | LLD-0001~0007 | 프로젝트 초기 1회, 다른 LLD의 기반 |
| 기능 API 묶음 | LLD-0008 그룹, LLD-0009 마이페이지 | 도메인 단위 CRUD |
| 복잡한 조회 분리 | LLD-0013 피드, LLD-0016 캐러셀 페이징 | 페이징 전략, 집계 로직 포함 |
| 아키텍처 구현 | LLD-0019 알림 아웃박스 | ADR → LLD로 구체화 |
| 스펙 정렬 | LLD-0020 API 스펙 정렬 | 기존 API 불일치 보정 |

---

### LLD 작성 방법: 섹션별로 무엇을 쓰는가

mody의 실제 LLD를 분석하면 각 섹션에 이런 내용이 들어간다:

**`## 1. 목적 / 배경`**: 왜 이 기능이 필요한가. 1~3문장. 클라이언트 요구사항 또는 서버 내부 필요.

**`## 2. 범위`**: In scope는 이번 PR에 넣을 것. Out of scope는 **의도적으로 뺀 것**을 명시.
```
Out of scope가 중요함 — 빠진 게 아니라 나중에 하기로 결정한 것임을 명시.
예) "기록 수정/삭제" → Out of scope로 명시하면 리뷰어가 왜 없는지 묻지 않음.
```

**`## 3. 인터페이스 / API`**: HTTP 메서드 + 경로 목록. 요청/응답 JSON 예시 (필드명, 타입, nullable 여부).
```
mody 패턴: http 코드블록으로 엔드포인트 목록 → 이어서 JSON 예시
응답 예시는 ApiResponse 래퍼 포함해서 작성.
```

**`## 4. 데이터 모델`**: 새로 만들거나 바뀌는 테이블/컬럼만. 엔티티 전체를 쓰지 않음.
```
WIDYU 주의: 엔티티 → widyu-domain, DTO → widyu-api/dto 위치 명시.
```

**`## 5. 처리 흐름`**: 번호 매긴 순서로. 트랜잭션 경계, @EventListener 사용 여부, Facade 유무 명시.
```
mody 패턴: "1. @CurrentMember가 인증 → 2. 검증 → 3. 저장" 형태.
```

**`## 6. 예외 / 에러 처리`**: 에러 코드와 발생 조건 목록. 새 에러 코드는 여기서 정의.

**`## 7. 인수조건 (Acceptance Criteria)`**: **이 항목들이 테스트 시나리오가 된다.**
```
mody 패턴:
- [x] 인증 회원이 그룹 코드를 발급받을 수 있다.   ← 구현 완료 후 체크
- [x] Swagger에 성공/주요 예외 응답이 반영된다.   ← Swagger도 인수조건에 포함
- [x] ./gradlew build가 통과한다.                ← 빌드 통과도 항목
```

**`## 8. 영향 범위 / 마이그레이션`**: 기존 코드가 바뀌는 부분. 새 테이블이면 "신규, 기존 영향 없음". MySQL ENUM 추가면 ALTER TABLE 명령.

**`## 9. 미결정 사항 (Open Questions)`**: 작성 시점에 결정 안 된 것. 구현 전에 채울 것. **추측으로 채우지 않음.**
```
mody 패턴: 구현 완료 후 "없음"으로 업데이트.
PR 본문의 Open Questions 섹션과 동기화.
```

---

### WIDYU에서 ADR/LLD 시작 순서

기존 코드가 이미 있는 상태에서 시작하므로, 백필 방식으로 접근한다:

**1단계 — 기반 ADR 백필 (현재 결정된 것 기록)**

```
ADR-0001: widyu-api + widyu-domain 멀티모듈 구조 (이미 결정됨, 기록만)
ADR-0002: 인증/인가 전략 (JWT + @ValidateFamilyAccess, 이미 적용됨, 기록만)
ADR-0003: DB/엔티티 설계 기준 (JPA 연관관계 O, FK 없음, enum 문자열 등)
ADR-0004: 이미지 업로드 방식 (Presigned URL, 이미 적용됨, 기록만)
ADR-0005: 커서 기반 페이징 전략 (이미 적용됨, 기록만)
```

**2단계 — ERD 백필 (현재 엔티티 기준)**

```
ERD-0001: 현재 widyu-domain 엔티티 전체 → Mermaid 다이어그램으로 기록
```

**3단계 — 신규 기능은 LLD 먼저 작성 후 구현**

```
새 기능 요청 발생
  → policy-checklist에 미결정 정책 추가
  → 팀 논의 후 정책 결정
  → 결정 방식이 중요하면 ADR 작성
  → LLD 작성 (ADR 링크, 인수조건 작성)
  → Claude 또는 Codex가 LLD 기반으로 구현
  → Codex review 스킬로 검수
  → PR 생성 (LLD 링크 포함)
```
