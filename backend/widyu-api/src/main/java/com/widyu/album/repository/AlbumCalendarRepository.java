    ...
    @Query("""
        ...
        AND (
            ...
            OR a.member.id IN (
                ...
                WHERE fc1.guardian.id = :memberId AND fc1.status = 'ACTIVE'
            )
            OR a.member.id IN (
                SELECT fc2.guardian.id FROM FamilyConnection fc2
                WHERE fc2.senior.member.id = :memberId AND fc2.status = 'ACTIVE'
            )
            OR a.member.id IN (
                SELECT fc3.guardian.id FROM FamilyConnection fc3
                WHERE fc3.senior.id IN (
                    SELECT fc4.senior.id FROM FamilyConnection fc4
                    WHERE fc4.guardian.id = :memberId AND fc4.status = 'ACTIVE'
                ) AND fc3.status = 'ACTIVE'
            )
        )
        ORDER BY a.id DESC
        LIMIT :limit
